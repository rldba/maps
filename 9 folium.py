...
df = pd.concat([df_bash_L2, df_svd_L2])

# добавляем данные из DataFrame в геопандадатафрейм
gpd_L2['ATM1000PEOPLE'] = gpd_L2['oktmo'].map(df.set_index('OKTMO')['ATM1000PEOPLE'])
gpd_L2['ATM'] = gpd_L2['oktmo'].map(df.set_index('OKTMO')['ATM'])
gpd_L2['PEOPLE'] = gpd_L2['oktmo'].map(df.set_index('OKTMO')['PEOPLE'])
...
#******************************** СЛОЙ С ТЕПЛОВОЙ КАРТОЙ L2 ************************************
# преобразую из geopandas в geo_json формат
geo_data_json_L2 = gpd_L2.to_json()
geo_data_json_L2 = json.loads(geo_data_json_L2)

#создаем слой хороплета
layer_choropleth_l2 = folium.Choropleth(
       geo_data = geo_data_json_L2,
       name = 'Плотность сети банкоматов (L2)',
       data = df,
       columns=['OKTMO', 'ATM1000PEOPLE'], #из DataFrame
       key_on='feature.properties.oktmo', # из GeoJSON
       bins = 4,
       fill_color='RdYlGn', #YlGn Blues BuPu
...
layer_choropleth_l2.add_to(m)
#*****************************************************************************************

m.keep_in_front(layer_choropleth_l2,layer_L2,layer_L3,layer_L4)
